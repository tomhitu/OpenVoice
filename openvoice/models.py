...
class ReferenceEncoder(nn.Module):
    ...
    def __init__(self, spec_channels, gin_channels=0, layernorm=True):
        ...
        if layernorm:
            ...
            self.layernorm.register_forward_hook(self.hook)
        else:
            self.layernorm = None
        for conv in self.convs:
            conv.register_forward_hook(self.hook)
        self.gru.register_forward_hook(self.hook)
        self.proj.register_forward_hook(self.hook)

    @staticmethod
    def hook(m, i, o):
        def print_shapes(name, data):
            if isinstance(data, torch.Tensor):
                print(f"Module: {m.__class__.__name__}, {name}: {data.shape}")
            elif isinstance(data, (tuple, list)):
                for idx, item in enumerate(data):
                    print_shapes(f"{name}[{idx}]", item)
            else:
                print(f"Module: {m.__class__.__name__}, {name}: {data}")
        print_shapes("Input", i)
        print_shapes("Output", o)

    def forward(self, inputs, mask=None):
        N = inputs.size(0)
        print(f"ReferenceEncoder inputs shape: {inputs.shape}, spec_channels: {self.spec_channels}")
        out = inputs.view(N, 1, -1, self.spec_channels)  # [N, 1, Ty, n_freqs]
        print(f"Initial out shape is {out.shape}")
